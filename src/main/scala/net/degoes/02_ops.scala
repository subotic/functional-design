...
object input_stream {
  ...
  final case class IStream(createInputStream: () => InputStream) { self => // best practice-ish: name the outer class self, because this is unambiguous as there could be anonymous classes inside this class and this would inside those mean the anonymous class
  ...
  lazy val fragments: List[IStream] =  fragments.foldLeft(IStream.empty){
    case (acc, cur) => acc ++ cur
  }
}

// Functional design and composability only works in the functional work, where a function is total, deterministic, free of side-effects.
trait FileChannel {
  def read: Int

  def write(i: Int): Unit
}
// we can wrap a functional domain around it though:
final case class FileChannelFactory(create: () => FileChannel)
...
object email_filter {
   ...
   */
  lazy val emailFilter1 = notJohn && bodyContainsDiscout && subjectContainsN95

  lazy val notJohn = EmailFilter.senderIs(Address("john@doe.com")).!
  lazy val bodyContainsDiscout = EmailFilter.bodyContains("discount")
  lazy val subjectContainsN95 = EmailFilter.subjectContains("N95")
}
...
object contact_processing {
  ...
  final case class SchemaMapping(map: ContactsCSV => MappingResult[ContactsCSV]) { self =>
    ...
    def +(that: SchemaMapping): SchemaMapping =
      ...
      // SchemaMapping(csv =>
      //   self.map(csv) match {
      //     case MappingResult.Success(value, warnings) => that.map(value)
      //     case MappingResult.Failure(errors)          => errors
      //   }
      // )
    ...
    def orElse(that: SchemaMapping): SchemaMapping =
      SchemaMapping(csv =>
        self.map(csv).orElse(that.map(_))
      )

      // self.map(csv) match {
      //   case MappingResult.Success(value, warnings) => value
      //   case MappingResult.Failure(errors)          => that.map(csv)
      // }
    }
  ...
  lazy val schemaMapping: SchemaMapping =
    ...
    SchemaMapping.combine("fname", "lname")("full_name")((f,l) =>)

  /*
  rename street => street_address
  rename email  => email_address
  rename postal => postal_code
  combine fname, lname => full_name (fname + " " + lname)
  reorder full_name to column 0
  */
...
object ui_events {
  ...
  final case class Listener(onEvent: GameEvent => Unit) { self =>
    ...
    def +(that: Listener): Listener =
      Listener {event =>
        // crashes in the first listener wont stop the second listener getting the event
        try self.onEvent(event) finally that.onEvent(event)  
      }
    ...
    def orElse(that: Listener): Listener =
      Listener {event =>
        try self.onEvent(event)
          ...
          case NonFatal(_) => that.onEvent(event) // NonFatal does not catch stackoverflow and such
        }
      }
...
object education {
  ...
  final case class Quiz(run: () => QuizResult) { self =>
    ...
    def check(f: QuizResult => Boolean)(ifPass: Quiz, ifFail: Quiz): Quiz =
      Quiz { () =>
        val result = self.run()
        result + (if (f(result)) ifPass.run() else ifFail.run())  
      }
  }
  object Quiz {
    private def grade[A](f: String => A, checker: Checker[A]): QuizResult =
      ...
      def fromIterable(quizez: Iterable[Quiz]): Quiz =
        quizes.foldLeft(Quiz.empty)(_ + _)
  ...
  lazy val exampleQuiz: Quiz =
    ...
    question2 +
    question3 +
    hardBonusQuestion.check(_.wrongPoints > 0)(easyBonusQuestion, Quiz.empty).bonus
    
    lazy val question1: Quiz = Quiz(Question.TrueFalse("Is coffee the best hot beverage on planet earth?", Checker.isTrue(10))) 
    lazy val question2: Quiz = ???
    lazy val question3: Quiz = ???
    lazy val hardBonusQuestion: Quiz = ???
    lazy val easyBonusQuestion: Quiz = ???
}
